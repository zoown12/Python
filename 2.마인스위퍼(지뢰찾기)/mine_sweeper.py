...
def num_of_bomb(field, x_pos, y_pos):
    """ 주위에 있는 폭탄 수를 반환한다 """
    count = 0
    for yoffset in range(-1, 2): #-1 0 1 범위 검사
        for xoffset in range(-1, 2): #-1 0 1 범위 검사
            xpos, ypos = (x_pos + xoffset, y_pos + yoffset)
            if 0 <= xpos < WIDTH and 0 <= ypos < HEIGHT and \
                field[ypos][xpos] == BOMB: #폭탄발견
                count += 1 #폭탄수 증가
    return count
        ...
        for xoffset in range(-1, 2):
            ...
            if 0 <= xpos < WIDTH and 0 <= ypos < HEIGHT and \
                ...
                if count == 0 and \
                    ...
                    open_tile(field, xpos, ypos) #재귀 호출

def main():
    """ 메인 루틴 """
    smallfont = pygame.font.SysFont(None, 36)#작은 글씨:숫자 표사
    largefont = pygame.font.SysFont(None, 72)#큰 글씨 : 메시지 표시
    message_clear = largefont.render("!!CLEARED!!",
                                     True, (0, 255, 225))
    message_over = largefont.render("GAME OVER!!",
                                    True, (0, 255, 225))
    message_rect = message_clear.get_rect()
    message_rect.center = (WIDTH*SIZE/2, HEIGHT*SIZE/2)# 화면 중앙에 표시
    game_over = False
    ...
    # 폭탄을 설치
    count = 0
    while count < NUM_OF_BOMBS: # 폭탄 개수만틈 반복
        xpos, ypos = randint(0, WIDTH-1), randint(0, HEIGHT-1)
        if field[ypos][xpos] == EMPTY: #비어 있는 곳의 경우
            field[ypos][xpos] = BOMB #랜덤 위치에 폭탄 배치함
            count += 1
    ...
    while True:
        ...
        # 선 그리기
        for index in range(0, WIDTH*SIZE, SIZE): # 가로
            pygame.draw.line(SURFACE, (96, 96, 96),
                             (index, 0), (index, HEIGHT*SIZE))
        for index in range(0, HEIGHT*SIZE, SIZE): #세로
            pygame.draw.line(SURFACE, (96, 96, 96),
